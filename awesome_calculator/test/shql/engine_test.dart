...
void main() {
  ...
  test('Calculate addition', () async {
    expect(await Engine.calculate('10+2'), 12);
  });
  test('Calculate addition and multiplication', () async {
    expect(await Engine.calculate('10+13*37+1'), 492);
  });
  ...
  test('Calculate addition and multiplication with parenthesis', () async {
    expect(await Engine.calculate('10+13*(37+1)'), 504);
  });
  ...
  test('Calculate addition, multiplication and subtraction', () async {
    expect(await Engine.calculate('10+13*37-1'), 490);
  });
  ...
  test(
    ...
    () async {
      expect(await Engine.calculate('10+13*37/2-1'), 249.5);
    },
  );
  ...
  test('Calculate modulus', () async {
    expect(await Engine.calculate('9%2'), 1);
  });

  test('Calculate equality true', () async {
    expect(await Engine.calculate('5*2 = 2+8'), true);
  });

  test('Calculate equality false', () async {
    expect(await Engine.calculate('5*2 = 1+8'), false);
  });

  test('Calculate not equal true', () async {
    expect(await Engine.calculate('5*2 <> 1+8'), true);
  });
  ...
  test('Evaluate match true', () async {
    expect(await Engine.calculate('"Super Man" ~  r"Super\\s*Man"'), true);
    expect(await Engine.calculate('"Superman" ~  r"Super\\s*Man"'), true);
    expect(await Engine.calculate('"Batman" ~  "batman"'), true);
  });

  test('Evaluate match false', () async {
    expect(await Engine.calculate('"Bat Man" ~  r"Super\\s*Man"'), false);
    expect(await Engine.calculate('"Batman" ~  r"Super\\s*Man"'), false);
  });

  test('Evaluate mismatch true', () async {
    expect(await Engine.calculate('"Bat Man" !~  r"Super\\s*Man"'), true);
    expect(await Engine.calculate('"Batman" !~  r"Super\\s*Man"'), true);
  });

  test('Evaluate mismatch false', () async {
    expect(await Engine.calculate('"Super Man" !~  r"Super\\s*Man"'), false);
    expect(await Engine.calculate('"Superman" !~  r"Super\\s*Man"'), false);
  });

  test('Evaluate in list true', () async {
    expect(
      await Engine.calculate('"Super Man" in ["Super Man", "Batman"]'),
      true,
    );
    expect(
      await Engine.calculate('"Super Man" finns_i ["Super Man", "Batman"]'),
      true,
    );
    expect(
      await Engine.calculate('"Batman" in  ["Super Man", "Batman"]'),
      true,
    );
    expect(
      await Engine.calculate('"Batman" finns_i  ["Super Man", "Batman"]'),
      true,
    );
  });
  ...
  test('Evaluate lower case in list true', () async {
    expect(
      await Engine.calculate('lowercase("Robin") in  ["batman", "robin"]'),
      true,
    );
    expect(
      await Engine.calculate('lowercase("Batman") in  ["batman", "robin"]'),
      true,
    );
  });
  ...
  test('Evaluate in list false', () async {
    expect(
      await Engine.calculate('"Robin" in  ["Super Man", "Batman"]'),
      false,
    );
    expect(
      await Engine.calculate('"Superman" in ["Super Man", "Batman"]'),
      false,
    );
  });
  ...
  test('Evaluate lower case in list false', () async {
    expect(
      await Engine.calculate('lowercase("robin") in  ["super man", "batman"]'),
      false,
    );
    expect(
  ...
  test('Calculate not equal false', () async {
    expect(await Engine.calculate('5*2 <> 2+8'), false);
  });
  ...
  test('Calculate less than false', () async {
    expect(await Engine.calculate('10<1'), false);
  });

  test('Calculate less than true', () async {
    expect(await Engine.calculate('1<10'), true);
  });
  ...
  test('Calculate greater than false', () async {
    expect(await Engine.calculate('1>10'), false);
  });

  test('Calculate greater than true', () async {
    expect(await Engine.calculate('10>1'), true);
  });
  ...
  test('Calculate some boolean algebra and true', () async {
    expect(await Engine.calculate('1<10 AND 2<9'), true);
    expect(await Engine.calculate('1<10 OCH 2<9'), true);
  });

  test('Calculate some boolean algebra and false', () async {
    expect(await Engine.calculate('1>10 AND 2<9'), false);
    expect(await Engine.calculate('1>10 OCH 2<9'), false);
  });

  test('Calculate some boolean algebra or true', () async {
    expect(await Engine.calculate('1>10 OR 2<9'), true);
    expect(await Engine.calculate('1>10 ELLER 2<9'), true);
  });

  test('Calculate some boolean algebra xor true', () async {
    expect(await Engine.calculate('1>10 XOR 2<9'), true);
    expect(await Engine.calculate('1>10 ANTINGEN_ELLER 2<9'), true);
  });

  test('calculate_some_bool_algebra_xor_false', () async {
    expect(await Engine.calculate('10>1 XOR 2<9'), false);
    expect(await Engine.calculate('10>1 ANTINGEN_ELLER 2<9'), false);
  });

  test('calculate_negation', () async {
    expect(await Engine.calculate('NOT 11'), false);
    expect(await Engine.calculate('INTE 11'), false);
  });

  test('calculate_negation with exclamation', () async {
    expect(await Engine.calculate('!11'), false);
  });

  test('Calculate unary minus', () async {
    expect(await Engine.calculate('-5+11'), 6);
  });

  test('Calculate unary plus', () async {
    expect(await Engine.calculate('+5+11'), 16);
  });

  test('Calculate with constants', () async {
    expect(await Engine.calculate('PI * 2'), 3.1415926535897932 * 2);
  });
  ...
  test('Calculate with functions', () async {
    expect(await Engine.calculate('POW(2,2)'), 4);
  });

  test('Calculate with two functions', () async {
    expect(await Engine.calculate('POW(2,2)+SQRT(4)'), 6);
  });

  test('Calculate nested function call', () async {
    expect(await Engine.calculate('SQRT(POW(2,2))'), 2);
  });

  test('Calculate nested function call with expression', () async {
    expect(await Engine.calculate('SQRT(POW(2,2)+10)'), 3.7416573867739413);
  });

  test('Calculate two expressions', () async {
    expect(await Engine.calculate('10;11'), 11);
  });
  ...
  test('Test assignment', () async {
    expect(await Engine.calculate('i:=42'), 42);
  });
